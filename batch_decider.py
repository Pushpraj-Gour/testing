...
def helper(data,batch_size):
    ...
    if last_batch_size==0:  # Perfect Batches
        batches = helper2(data,batch_size)
    
    else:  # Last Batch Problem
        total_batches = (total_element//batch_size) +1

        if last_batch_size<=(total_batches-1) and last_batch_size<=(batch_size//2): 
            ...
            i =0
            while i<len(data):
                batches.append(data[i:i+adj_batch_size])
                total_batches-=1
                total_element-=adj_batch_size
                i+=adj_batch_size

                # Checking if the remaining element can be distributed equally to the remaining batches
                if total_element>0 :
                      if total_element % total_batches==0:
                            adj_batch_size = total_element//total_batches
...
def test_helper_1():
    ...
    assert helper(list(range(1,29)),10) == [list(range(1,11)),list(range(11,20)),list(range(20,29))]

test_helper_1()


# print(helper(list(range(1,29)),10))   